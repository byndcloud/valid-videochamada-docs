...
```javascript
<script src='https://beta.server.minhasala.app/external_api.js'></script>
```
...
* Android: `intent://beta.server.minhasala.app/test123#Intent;scheme=org.omni.meet;package=org.omni.meet;end`
* iOS: `org.omni.meet://beta.server.minhasala.app/test123`

This works with custom servers too, just replace `beta.server.minhasala.app` with your custom server URL.

## Creating the Omni Meet API object
...
* **domain**: The domain used to build the conference URL (e.g., **`beta.server.minhasala.app`**).
...
const api = new OmniMeetExternalAPI(domain, options);
```
...
const api = new OmniMeetExternalAPI(domain, options);
```
To pass a JWT token to Omni Meet use the following:
...
var api = new OmniMeetExternalAPI(domain, options);
```
...
Returns the IFrame HTML element which is used to load the Omni Meet conference:
...
**NOTE:** Omni recommends removing the conference before the page is unloaded.
...
The `OmniMeetExternalAPI` object implements the [EventEmitter] API for emitting and listening for events.
